...
V3: Uses option letters (a,b,c,d) as primary grouping structure.
Groups every 4 consecutive option rows into one question.
"""
...
HEADER_ONLY = set(sp for sp, _, _ in SECTIONS if sp not in CONTENT_SECTION_PAGES)
...
def main():
    print("=" * 70)
    print("Extractor v3 - Option-letter based grouping")
    print("=" * 70)
    
    # Step 1: Collect ALL rows with their page index
    all_rows = []
    ...
    # Step 2: Group into questions using option letters
    # Strategy: Each row has an option letter (a/b/c/d) in col[2].
    # Every group of 4 rows starting with 'a' forms one question.
    # The 'a' row (or one of the 4 rows) will have the question number 
    # and enunciado in cols [0] and [1].
    ...
    while i < len(all_rows):
        ...
        # Look for start of a question: option 'a'
        if opt_l != 'a':
            i += 1
            continue
        
        # Found option 'a'. Collect this and the next 3 rows (b, c, d)
        q_rows = [(pi, row)]
        j = i + 1
        ...
        for exp_l in expected_letters:
            if j < len(all_rows):
                ...
                if next_opt == exp_l:
                    ...
                    # Missing option - still try to continue
                    break
        ...
        # Build question from these rows
        ...
        # Find the enunciado - it's on the row that has the question number
        enunciado = ""
        q_num = ""
        ...
        # If no enunciado found on option rows, check the row before 'a'
        ...
        if not enunciado:
            # Try to get it from the 'a' row anyway
            en = cell(q_rows[0][1])
            if en and len(en) > 5:
                enunciado = en
        ...
        if q['question'] and len(q['options']) >= 3 and q['correct']:
            ...
            if len(q['options']) < 3:
                reasons.append(f'{len(q["options"])} opts')
            if not q['correct']:
